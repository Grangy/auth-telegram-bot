...
try {
    ...
    bot.on('error', (error) => {
        errorCount++;
        logger.error(`Ошибка Telegram бота (${errorCount}/${maxErrors}):`, error.message);
    ...
    logger.error('Критическая ошибка инициализации бота:', error.message);
    process.exit(1);
}
...
function cleanupOldSessions() {
    ...
    if (cleaned) {
        saveDatabase(db);
        logger.info('Очищены устаревшие сессии');
    }
}
...
function cleanupInactiveCodes() {
    ...
    if (cleaned) {
        saveDatabase(db);
        logger.info('Очищены устаревшие коды');
    }
}
            ...
            if (longTermSession && longTermSession.expiresAt > Date.now()) {
                ...
                for (const [sessionId, session] of Object.entries(db.sessions)) {
                    if (session.phone === longTermSession.phone && session.authorized) {
                        ...
                        saveDatabase(db);
                        logger.info(`Обновлен socketId для сессии ${sessionId}`);
    ...
    socket.on('requestAuth', async (data) => {
        ...
        if (existingUser) {
            ...
            if (existingUser.telegramUserId) {
                ...
                logger.info(`Код отправлен в Telegram пользователю ${existingUser.telegramUserId}: ${smsCode}`);
            }
        ...
        try {
            ...
            logger.info(`Создан ключ авторизации: ${authKey} для телефона: ${phone}`);
            logger.info(`Ссылка для авторизации: ${authLink}`);
        } catch (error) {
        ...
        for (const [phone, codeData] of Object.entries(db.smsCodes || {})) {
            if (!codeData.used && 
                ...
                if (user) {
                    ...
                    logger.info(`Авторизация по коду успешна для ${phone} (${user.name})`);
                    return;
                }
            }
        }
    ...
    socket.on('extendSession', (data) => {
        if (data && data.sessionToken) {
            ...
            if (extended) {
                ...
                logger.info(`Сессия продлена для токена: ${data.sessionToken}`);
            } else {
...
bot.on('contact', (msg) => {
    ...
    logger.info(`Получен контакт от пользователя ${userId}: ${phoneNumber}`);
    ...
    for (const [authKey, authData] of Object.entries(db.authKeys)) {
        if (!authData.used && 
            ...
            if (sessionData) {
                ...
                logger.info(`Авторизация успешна для ${phoneNumber} (${userName})`);
                return;
            }
        }
    }
...
bot.on('message', (msg) => {
    ...
    if (messageText && messageText.match(/^\d{4}$/)) {
        ...
        logger.info(`Получен SMS код от пользователя ${userId}: ${smsCode}`);
        ...
        for (const [phone, codeData] of Object.entries(db.smsCodes || {})) {
            if (!codeData.used && codeData.code === smsCode) {
                ...
                logger.info(`SMS авторизация успешна для ${phone} (${msg.from.first_name})`);
                return;
            }
        }
    ...
    if (messageText && messageText.match(/^\+?[1-9]\d{1,14}$/)) {
        ...
        logger.info(`Получен номер телефона от пользователя ${userId}: ${phoneNumber}`);
        ...
        for (const [authKey, authData] of Object.entries(db.authKeys)) {
            if (!authData.used && 
                ...
                if (sessionData) {
                    ...
                    logger.info(`Авторизация успешна для ${phoneNumber} (${msg.from.first_name})`);
                    return;
                }
            }
        }
...
bot.on('polling_error', (error) => {
    logger.error('Ошибка polling Telegram бота:', error);
});
...
setInterval(() => {
    ...
    if (cleaned) {
        saveDatabase(db);
        logger.info('Очищены устаревшие ключи авторизации и SMS коды');
    }
}, 5 * 60 * 1000);
